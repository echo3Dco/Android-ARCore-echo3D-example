...
public class HelloArActivity extends AppCompatActivity implements GLSurfaceView.Renderer {
  ...
  public void onSurfaceCreated(GL10 gl, EGLConfig config) {
    ...
    try {
      ...
      if (!Config.OFFLINE_MODE) {
        Entry entry = CMS.db.getEntries().iterator().next();
        ModelHologram modelHologram = (ModelHologram) entry.getHologram();
        //virtualObject.createOnGlThread(/*context=*/ this, "model.obj", "model.png");
        virtualObject.createOnGlThread(/*context=*/ this, modelHologram.getFilename(), modelHologram.getTextureFilename());
      }
      else
  ...
  private float getScaleFactor(){
    ...
    try {
      Scanner scanner = new Scanner(CMS.getFile(((ModelHologram) CMS.db.getEntries().iterator().next().getHologram()).getFilename()));
      // Read the file line by line
      int lineNum = 0;
      while (scanner.hasNextLine()) {
        String line = scanner.nextLine();
        lineNum++;
        if (line.toLowerCase().contains("cake")) {
          scaleFactor = 0.005f;
          break;
        } else if (line.toLowerCase().contains("plane")) {
          scaleFactor = 0.05f;
          break;
        } else if (lineNum > 15)
          break;
